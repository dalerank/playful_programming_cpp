...
```text
===================================================================
HANDLE / TOKEN: РАЗРЕШЕНИЕ ПРОБЛЕМЫ ABA ЧЕРЕЗ ПОКОЛЕНИЯ (GENERATION)
===================================================================

Структура токена (32-битный Handle):
┌───────────────────────────────┬───────────────────────────────┐
│     generation (16 бит)       │       slot_index (16 бит)     │
└───────────────────────────────┴───────────────────────────────┘
 Например: { gen: 1, slot: 2 }   ──> указывает на слот 2 поколения 1

Пул сущностей (Entity Pool):
  Слот 0: [ gen: 1 | Monster A ]
  Слот 1: [ gen: 1 | Monster B ]
  Слот 2: [ gen: 1 | Monster C ]  ◄── Башня держит токен { gen: 1, slot: 2 }

Сценарий ABA (смерть монстра C и спавн монстра D в тот же слот):

 1. Монстр C убит:
    Слот 2 освобождён, счётчик поколения слота увеличивается:
    Слот 2: [ gen: 2 | ─── СВОБОДЕН ─── ]

 2. Заспавнен новый Монстр D на другом конце карты:
    Слот 2 переиспользован для Монстра D с новым поколением:
    Слот 2: [ gen: 2 | Monster D ]  ──> Новый токен: { gen: 2, slot: 2 }

 3. Башня пытается выстрелить по старому токену { gen: 1, slot: 2 }:
    Проверка валидности:
      token.slot_index ──► Слот 2
      token.generation (1) == slot.generation (2) ? ──► FALSE (НЕДЕЙСТВИТЕЛЕН!)

    Результат:
      Башня видит, что старый монстр C умер, сбрасывает цель и не стреляет
      в нового монстра D через всю карту, хотя слот в памяти тот же самый.
```

Практическое правило для нас получается такое - сырой указатель, ссылка или индекс это отличный способ сказать «где», но работа они только внутри одного состояния системы, пока вы сами гарантируете, что никто не спавнится, не умирает и не перекладывает массив. Как только ссылка должна пережить кадр, попасть в очередь событий, в сохранение или на другой поток, она обязана превратиться в токен, отвечающий на «кто», будь то id с поколением, первичный ключ или handle с проверкой. Башня из начала главы, держи она такой токен, просто не нашла бы свою текущую цель и взяла бы следующую, вместо того чтобы прилежно стрелять по старому «адресу».
